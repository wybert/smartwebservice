...
class Tag(Sprite):
    ...
    """
    def __init__(self, tag, initial_position, rotation=0, fontname='simhei.ttf', fontzoom=5):
...
##    return minsize + (count*1.0)/(maxcount-mincount)*(maxsize-minsize)
   
    return minsize + (maxsize - minsize) * ((count*1.0-mincount*1.0) / (maxcount*1.0 - mincount*1.0))**2.0

def make_tags(wordcounts, minsize=5, maxsize=40, colors=None, scalef=defscale):
    """
...
def _draw_cloud(tag_list, surface, vertical=True, fontname='simhei.ttf', fontzoom=5, rectangular=False):
...
def create_tag_image(tags, file, size=(800, 600), background=(255, 255, 255), vertical=True, crop=True, fontname='simhei.ttf', fontzoom=2, rectangular=False):
    """
...
def create_html_data(tags, size=(600, 400), fontname='simhei.ttf', fontzoom=2, rectangular=False):
    """
